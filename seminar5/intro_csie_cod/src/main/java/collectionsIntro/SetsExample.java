...
//    TreeSet: A set that stores unique elements in sorted order, either by natural ordering or a specified comparator.
    ...
    {
        //set= colectie fara duplicate, nu garanteaza ordinea, nu are index
        Set<String> s = new HashSet<>();
        s.add("banana");
        s.add("apple");
        s.add("banana");    //este ignorat, pt ca Set verifica daca elementul exista deja, folosind hashCode() si equals() din clasa String
        System.out.println(s);

        Set<Dog> d = new HashSet<>();
        d.add(new Dog("john1",3));
        d.add(new Dog("john1",3));  //este ignorat, pt ca Set verifica daca elementul exista deja, folosind hashCode() si equals() din clasa Dog
        d.add(new Dog("john2",8));
        System.out.println(d);

        /// to act like a hashset we need to implement equals and hashcode
       //!!! A HashSet itself cannot be sorted because it is unordered by design.
        //dar il poti sorta daca folosesti direct TreeSet sau il convertesti:
